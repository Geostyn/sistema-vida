...
def run_daytrade_backtest(mt5_connector, symbol: str,
    ...
    for i in range(300, n - 2):
        ...
        # Costes en R: spread + comisión sobre la distancia del SL
        cost_r = (spread_usd / risk) + (comm_lot / (risk * 100.0))
        ...
        eq        = equity[-1]
        lots      = max(0.01, np.floor((eq * 1.08 * risk_pct) / (risk * 100.0) / 0.01) * 0.01)
        risk_eur  = lots * risk * 100.0 / 1.08
        ...
        trades.append({
            ...
            "tp1":       round(tp1, 2), "sl_dist": round(risk, 2),
...
def _report(trades, equity, discards, date_from, date_to, balance0, symbol,
    ...
    out = os.path.join(LOGS_DIR, f"backtest_daytrade{suffix}.json")
...
if __name__ == "__main__":
    ...
    run_daytrade_backtest(conn, cfg["symbols"]["primary"], dt_from, dt_to,
